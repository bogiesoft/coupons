...
$(function(){
	$(".first").bind("click",function(){
		...
		$(".businessCon").slideToggle(0,function(){
			//窗口高度自适应
    ...
    $(".parentBox").click(function(){
        ...
        var ddList = $(this).parent().siblings("dd").find(".childBox");
    	var arr=[];
    	if ($(this).is(":checked")) {
    		$parentNode.prop("checked",true);
            ddList.prop("checked",true);
    	}else{
            ddList.prop("checked",false);            
        }
    	var store_count = 0;
      	$('.childBox').each(function(){
      		if($(this).is(':checked')){
    ...
    $(".comList dt .rbArrow").click(function() {
        $(this).parent().parent().toggleClass("closeFolder");
        $(this).parent().siblings("dd").toggle();
    })

    $(".comList dd .childBox").click(function() {
        var childList = $(this).parents(".child1").find(".childBox");
        var parent = $(this).parent().siblings("dt").find(".parentBox");
        var arr = [];
        ...
        if (arr.length === childList.length) {
