	...
	<div class="pop_con popStoreSel">
        ...
        <?php echo CHtml::beginForm('','get'); ?>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tableH">
                ...
                <?php if(!empty($store_lists)){ $count=0;?>
                
                   <?php foreach ($store_lists as $k => $v){ $count++;?>
                   <tr>
                		<td>
                	  		<input type="checkbox" <?php if(in_array($v->id, $store_id_arr)) {echo 'checked';}?> name="store_id[]" value="<?php echo $v->id; ?>">
                		</td>
                    	<td align="left">
            ...
            <div class="btnWrap">
            	...
            	<div class="total">已选<span id="change"><?php echo count($store_id_arr)?></span>家，共<?php echo $pages -> getItemCount(); ?>家</div>
...
$(function(){
	var i = 0;
	$.ajax({
		 url : '<?php echo $this->createUrl('getStoreList'); ?>',
		 type:'post',
	      ...
	      $.ajax({
				 ...
				 success:function(data){
					 ...
					 $.each(res,function(inx,item){
						 ...
						 $.ajax({
								 url : '<?php echo $this->createUrl('delChooseStore'); ?>',
