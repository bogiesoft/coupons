        	...
        	<div class="<?php if($list['refund'][0]['if_return']==IF_RETURN_NO){echo "step stepone";}else{echo "step";}?>">
                ...
                <!--'第一步是step01；第二步为step02；第三步step03；第四步step04-->  
                	<li>
                        ...
                        <span class="stepTime"><?php echo date('Y.m.d H:i:s',strtotime($list['refund'][count($list['refund'])-1]['create_time']));?></span>
                    </li>
                    <li>
                        ...
                        <span class="stepTime"><?php if(!empty($list['refund'][0]['refuse_refund_remark'])||count($list['refund'])>1){echo date('Y.m.d H:i:s',strtotime($list['refund'][0]['create_time']));}?></span>
                    </li>
                    <li>
                        ...
                        <span class="stepTime"><?php if ($list['status'] == ORDER_REFUND_STATUS_RETURN_ISSUED||$list['status']==ORDER_REFUND_STATUS_RETURN_RECEIPT){echo date('Y.m.d H:i:s',strtotime($list['refund'][0]['create_time']));}?></span>
                    </li>
                    <li>
                    	<span class="stepName <?php if ($list['status'] == ORDER_REFUND_STATUS_REFUND_SUCCESS){echo 'color';} ?>">退款完成</span>
                        <span class="stepTime"><?php if($list['status'] == ORDER_REFUND_STATUS_REFUND_SUCCESS){echo date('Y.m.d H:i:s',strtotime($list['refund'][0]['agree_refund_time']));}?></span>
                    </li>
                </ul>
                ...
                <!--不需要退货-->
                    <ul class="<?php if($list['status']==ORDER_REFUND_STATUS_APPLY_REFUND_NORETURN&&count($list['refund'])<=1){echo 'step01';}
                        ...
                        <li>
                            ...
                            <span class="stepTime"><?php echo date('Y.m.d H:i:s',strtotime($list['refund'][0]['create_time']));?></span>
                        </li>
                        <li class="end">
                            <span class="stepName <?php if ($list['status'] == ORDER_REFUND_STATUS_REFUND_SUCCESS){echo 'step04';} ?>">退款完成</span>
                            <span class="stepTime"><?php if(!empty($list['refund'][0]['agree_refund_time'])){echo date('Y.m.d H:i:s',strtotime($list['refund'][0]['agree_refund_time']));}?></span>
                        </li>
                    </ul>
                <?php }?>
            </div>
			...
			<div class="orderL">
                ...
                <dl class="shopPic">
                    ...
                    <dd>
                        ...
                        <p><?php echo $list['color']." "; ?><?php echo $list['size']; ?></p>
                    </dd>
                </dl>
<!--                <div class="filed">-->
...
<!--                </div>-->
                ...
                <div class="filed">
                    ...
                    <span class="text"><?php echo date('Y.m.d H:i:s',strtotime($list['order_info']['pay_time'])); ?></span>
                </div>
                <div class="filed">
            	...
            	<div class="title">
                 ...
                 <div class="con">
                   <?php if($list['status'] == ORDER_REFUND_STATUS_APPLY_REFUND_RETURN){ ?><!-- 订单sku状态是申请退款需退货 -->
                  	...
                  	<?php }elseif ($list['status'] == ORDER_REFUND_STATUS_APPLY_REFUND_NORETURN){?><!-- 订单sku状态是申请退款无需退货 -->
                  	<a href="<?php echo Yii::app()->createUrl('mCenter/dshopActivist/AgreeRefund',array('order_id'=>$order_id,'order_sku_id'=>$list['id'],'order_status'=>$order_status)); ?>" onclick="return confirm('确定同意退款吗');" class="btn_com_blue">同意退款</a>
                  	<a href="javascript:;" onclick="refuseReturn()" class="btn_comCancel">拒绝退款</a>
                  	<?php }else if($list['status']==ORDER_REFUND_STATUS_RETURN_ISSUED){?>
                        <a href="<?php echo Yii::app()->createUrl('mCenter/dshopActivist/AgreeTake',array('order_id'=>$order_id,'order_sku_id'=>$list['id'],'order_status'=>$order_status)); ?>" onclick="return confirm('确定同意收货吗');" class="btn_com_blue">同意收货</a>
                        <a href="<?php echo Yii::app()->createUrl('mCenter/dshopActivist/RefuseTake',array('order_id'=>$order_id,'order_sku_id'=>$list['id'],'order_status'=>$order_status)); ?>" onclick="return confirm('确定拒绝收货吗');" class="btn_comCancel">拒绝收货</a>
                    <?php }?>
                 </div>
                 <div class="border"></div>
                 <div class="filed">
        ...
        <div class="DR_record">
            ...
            <?php foreach($list['refund'] as $k=>$v){?>
                ...
                <?php if($list['refund'][$k]['status']==ORDER_REFUND_STATUS_FINANCIAL_PLAY||$list['refund'][$k]['status']==ORDER_REFUND_STATUS_REFUND_SUCCESS
                    ...
                    <dl>
                        <dt><strong>商家</strong><span class="time"><?php echo date('Y.m.d H:i:s',strtotime($list['refund'][$k]['refund_time']));?></span></dt>
                        <dd>
                            <p>同意退款给买家，本次维权结束</p>
                            <div class="filed">
                ...
                <?php if(!empty($list['refund'][$k]['refuse_refund_remark'])){?>
                    <!--拒绝退款理由不为空-->
                    <dl>
                        <dt><strong>商家</strong><span class="time"><?php echo date('Y.m.d H:i:s',strtotime($list['refund'][$k]['create_time']));?></span></dt>
                        <dd>
                ...
                <?php if($list['refund'][$k]['status']==ORDER_REFUND_STATUS_RETURN_ISSUED){
                    if($list['refund'][$k]['if_return']==IF_RETURN_YES){?>
                        <dl>
                            <dt><strong>买家</strong><span class="time"><?php echo date('Y.m.d H:i:s',strtotime($list['refund'][$k]['create_time']))?></span></dt>
                            <dd>
                                <p>已退货，等待商家确认收货</p>
                                <div class="filed">
                                    <div class="label inlineBlock">物流名称：</div>
                ...
                <?php if(!empty($list['refund'][$k]['apply_refund_time'])&&($list['refund'][$k]['status']==ORDER_REFUND_STATUS_APPLY_REFUND_NORETURN||$list['refund'][$k]['status']==ORDER_REFUND_STATUS_APPLY_REFUND_RETURN)){?>
                    <dl>
                        <dt><strong>买家</strong><span class="time"><?php echo date('Y.m.d H:i:s',strtotime($list['refund'][$k]['apply_refund_time']))?></span></dt>
                        <dd>
                            <p>发起了退款申请，等待商家处理</p>
                            <div class="filed">
...
<div class="popWrap" id="pop" style="width:655px; display:none">
	<div class="pop_con pAddress">
        ...
        <?php echo CHtml::beginForm(Yii::app()->createUrl('mCenter/dshopActivist/addRefundAddress')); ?>
...
<div class="popWrap" id="pop_refuse" style="width:450px; display:none">
    <div class="pop_con pAddress">
        ...
        <?php echo CHtml::beginForm(Yii::app()->createUrl('mCenter/dshopActivist/RefuseRefund')); ?>
        <div class="pop_content">
            <!--<div class="filed">
            ...
            if($list['status'] == ORDER_REFUND_STATUS_APPLY_REFUND_NORETURN)
            {
                echo REQUEST_ORDER_STATUS_PAID;
            }else if($list['status'] == ORDER_REFUND_STATUS_APPLY_REFUND_RETURN){
                echo ORDER_REFUND_STATUS_AGREE_RETURN;
            }
            ?>">
