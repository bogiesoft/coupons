...
<div class="kkfm_r_inner">
    ...
    <div class="shop_menu">
        <ul>
            ...
            <li><a href="<?php echo Yii::app()->createUrl('mCenter/dshopGroup/ShopGroupList'); ?>">商品分组</a></li>
        </ul>
    </div>
    ...
    <div class="contant shopList">
        <div class="cz">
            <a href="<?php echo Yii::app()->createUrl('mCenter/dshopProduct/addProductOfCategory') ?>"
                ...
                <?php if (!empty($product)) { ?>
                    <?php foreach ($product as $k => $v) { ?>
                        <tr>
                            ...
                            <td>0</td>
                            <td>
                                <a href="<?php echo Yii::app()->createUrl('mCenter/dshopProduct/editProductOfCategory', array('pro_id' => $v->id)) ?>"
                                   class="blue">编辑</a> -
                                <a href="<?php echo Yii::app()->createUrl('mCenter/dshopProduct/delProduct', array('pro_id' => $v->id)) ?>"
...
<div class="popWrap change_group_pop" id="pop" style="display: none">
    <div class="pop_con">
        <div class="title">修改分组<a href="<?php echo Yii::app()->createUrl('mCenter/dshopGroup/ShopGroupList') ?>"
                                  class="manage_btn">管理</a></div>
        <div class="pop_content popStored">
            <ul>
                <?php foreach ($grouplist as $key => $value) { ?>
                    <li><input type="checkbox" name="change_group" id="group1" value="<?php echo $key ?>"><label
                            for="group1"><?php echo $value ?></label></li>
                <?php } ?>
            </ul>
        </div>
        <div class="btn">
...
//                     $("select#shop_product_status option[value=pro_status]").attr('selected', 'true');
            ...
            if (choosed_groupid != '') {
                ...
                $("select#shop_group").val(choosed_groupid);
            }
            if (key_word != '') {
    ...
    $('#undercarriage').click(function (e) {
        ...
        if (confirm('确认下架吗')) {
            ...
            $.ajax({
                url: '<?php echo(Yii::app()->createUrl('mCenter/dshopProduct/UnderCarriage'));?>',
                data: {content: tmp},
                dataType: "json",
                type: 'get',
    ...
    $('#upcarriage').click(function (e) {
        ...
        if (confirm('确认上架吗')) {
            ...
            $.ajax({
                url: '<?php echo(Yii::app()->createUrl('mCenter/dshopProduct/UpCarriage'));?>',
                data: {content: tmp},
                dataType: "json",
                type: 'get',
                success: function (data) {
    ...
    $('#save').click(function (e) {
        ...
        $.ajax({
            url: '<?php echo Yii::app()->createUrl('mCenter/dshopProduct/ChangeGroup')?>',
    ...
    $('#search').click(function (e) {
        ...
        if (arrow == 'arrowUp')
            ...
            arrow = 'arrowUp';
        var url = '<?php echo Yii::app()->createUrl('mCenter/dshopProduct/ProductList')?>?pro_status=' + pro_status + '&group_id=' + group_id + '&key_word=' + key_word + '&shop_group=' + shop_group + '&arrow=' + arrow + '&arrow_type=' + arrow_type;
        window.location = url;
    });
    ...
    $('#volume').click(function (e) {
        ...
        var arrow_type = $('#arrow_type').val();
        arrow_type = 'volume';
        var url = '<?php echo Yii::app()->createUrl('mCenter/dshopProduct/ProductList')?>?pro_status=' + pro_status + '&group_id=' + group_id + '&key_word=' + key_word +
    ...
    function delMore() {
        ...
        if (confirm('确认删除吗')) {
            ...
            $.ajax({
                url: '<?php echo(Yii::app()->createUrl('mCenter/dshopProduct/delMoreProduct'));?>',
                data: {content: tmp},
                dataType: "json",
                type: 'get',
                success: function (data) {
