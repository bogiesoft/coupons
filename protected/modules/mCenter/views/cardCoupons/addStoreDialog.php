...
$(function(){
	var i = 0;
	$.ajax({
		 url : '<?php echo Yii::app()->createUrl('mCenter/cardCoupons/getStoreList'); ?>',
		 type:'post',
...
// 			sumChecked ++; //该页勾选总数
// 		}else{
// 			sumNochecked ++; //该页未勾选总数
// 		}		
// 	    });
// 	if(sumChecked <=2 && sumNochecked==0){ //已选门店数等于每页总门店数    全选勾选
// 	     $("#all_check").prop("checked",true);
//     }
...
// 	    });
	      ...
	      $.ajax({
				 ...
				 success:function(data){
					 ...
					 $.each(res,function(inx,item){
						 $.ajax({
								 url : '<?php echo Yii::app()->createUrl('mCenter/cardCoupons/delChooseStore'); ?>',
...
function send()
{
	...
	store_arr.pop();// 移除最後一元素(最后一个元素为空)
	store_arr = unique(store_arr); //去掉重复的元素
// 	for (i=0;i<store_arr.length ;i++ )   
//     {   
//         alert(store_arr[i]); 
//     } 
// 	$("input[name='store_id[]']:checkbox").each(function(){
// 		if($(this).prop("checked")){
// 			store_arr[i] = $(this).val();
// 			i++;
// 		}
// 	});
