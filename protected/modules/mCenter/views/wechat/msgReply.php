    ...
    <div class="reply">
    	<div class="nr">
			<?php echo CHtml::beginForm();?>
				...
				<div class="nr-con">
					<div id="content" <?php if(!empty($model['content']) || empty($model['material_id'])) { ?>style="display: show"<?php }else {?>style="display: none"<?php }?>>
						<?php echo CHtml::textArea('Reply[content]', $model['content'], array())?>
					</div>
					<div id="material" <?php if(!empty($model['material_id'])) { ?>style="display: show"<?php }else {?>style="display: none"<?php }?>>
    	...
    	<div class="btn">
			...
			<a href="<?php echo Yii::app()->createUrl('mCenter/wechat/delAutoReply', array('reply_id'=>$model['id'], 'type'=>REPLY_TYPE_MSG))?>" class="btn_com_gray" onclick="return confirm('确认删除吗？');">删除</a>
		</div>
