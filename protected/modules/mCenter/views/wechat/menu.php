<body>
	...
	<div class="kkfm_r_inner">
	        		...
	        		<?php foreach ($arr as $key => $value){?>
	        			<dl class="layer tile" data-force="30">
		                	<dt class="tile__name" val="<?php echo $key?>"><?php echo $value['menu_name']?>
		                    	<div class="shade">
		                        	<a href="<?php echo Yii::app()->createUrl('mCenter/Wechat/Menu', array('id'=>$key))?>" class="edit"></a>
		                            <a href="<?php echo Yii::app()->createUrl('mCenter/Wechat/delMenu', array('id'=>$key))?>" class="del" onclick="return confirm('确认删除吗？');"></a>
		                        </div>
		                    </dt>
		                    <dd class="tile__list">
			                    <?php if (!empty($value['son'])) { ?>
			                		<?php foreach ($value['son'] as $k => $v){?>
					                	<div class="subMenu" val="<?php echo $k?>"><?php echo $v['menu_name']?>
					                    	<div class="shade">
					                        	<a href="<?php echo Yii::app()->createUrl('mCenter/Wechat/Menu', array('id'=>$k))?>" class="edit"></a>
					                            <a href="<?php echo Yii::app()->createUrl('mCenter/Wechat/delMenu', array('id'=>$k))?>" class="del" onclick="return confirm('确认删除吗？');"></a>
					                        </div>
					                    </div>
					                <?php }?>
			                    <?php }?>
	        ...
	        <div class="menu-mod">
	          <?php echo CHtml::beginForm(Yii::app()->createUrl('mCenter/Wechat/Menu'), 'post' ,array('name' => 'addmerchant'));?>
	        	<div class="menu-con">
...
$("#suresort").click(function(){
	...
	$.ajax({
		url : "<?php echo Yii::app()->createUrl('mCenter/Wechat/sortMenu')?>",
		data : {srotStr : str},
		type: 'post',
		dataType: "json",
		async : false,
...
$("#publish").click(function(){
	$.ajax({
        url: "<?php echo Yii::app()->createUrl('mCenter/Wechat/publishMenu') ?>",
