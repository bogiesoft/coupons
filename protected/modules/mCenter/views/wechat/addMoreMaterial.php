    ...
    <form id="material_form" method="post" action="?">
    	<div class="left">
            <div class="item mutil">
                <div class="imgSet" id="big01">
                    ...
                    <input type="hidden" value="" name="MaData[Data0][Title]">
                    <input type="hidden" value="" name="MaData[Data0][Url]">
                    <input type="hidden" value="" name="MaData[Data0][ImgPath]">
                    <input type="hidden" value="" name="MaData[Data0][inputContent]">
                    <input type="hidden" value="1" name="MaData[Data0][type]">
                    <input type="hidden" value="1" name="MaData[Data0][jumpType]">
                    <input type="hidden" value="" name="MaData[Data0][ImgAllPath]">
                </div>
                
                <div class="list" id="msg01">
                    <div class="txt">标题</div>
                    ...
                    <input type="hidden" value="" name="MaData[Data1][Title]">
                    <input type="hidden" value="" name="MaData[Data1][Url]">
                    <input type="hidden" value="" name="MaData[Data1][ImgPath]">
                    <input type="hidden" value="" name="MaData[Data1][inputContent]">
                    <input type="hidden" value="1" name="MaData[Data1][type]">
                    <input type="hidden" value="1" name="MaData[Data1][jumpType]">
                    <input type="hidden" value="" name="MaData[Data1][ImgAllPath]">
                </div>
                <div class="add"><a href="#add" num="2" onClick="myadd(this)">&nbsp;</a></div>
                <div class="remark">还可以添加<em id="limitNum">4</em>篇文章</div>
            </div>
            <div class="btn">
            	<input type="button" class="btn_com_blue" value="提交保存" onclick="save()"/>
                <a href="<?php echo Yii::app()->createUrl('mCenter/wechat/materialList')?>" class="btn_com_gray">返回</a>
            </div>
        </div>
		...
		<?php echo CHtml::beginForm();?>
        	<div class="mod-form">
                ...
                <div class="label">
                	<label>详情</label><br>
                    <?php echo CHtml::radioButtonList('Material[jump_type]', '1', array(MATERIAL_CONTENT_TEXT => $GLOBALS['MATERIAL_JUMP_TYPE'][MATERIAL_CONTENT_TEXT], MATERIAL_CONTENT_URL => $GLOBALS['MATERIAL_JUMP_TYPE'][MATERIAL_CONTENT_URL]), array('separator'=>''))?>
		        </div>
...
// 					) );?>
					<?php echo CHtml::textArea('Material[content]',isset($_POST['Material']['jump_type'])?$_POST['Material']['jump_type']:'',array('style'=>'width:1000px; height:500px')); ?>
                   	<!-- <script id="content" type="text/plain" style="width:1000px;height:500px;"></script> -->
                </div>
                <div class="arrow"></div>
            </div>
    ...
    $('#upload').uploadify({
        ...
        onUploadSuccess: function (file, data, response) {//一个文件上传成功后的响应事件处理
           ...
           // $(".img").attr('src','<?php //echo IMG_GJ_LIST?>'+fileName);
    		...
    		$(cur_id+' input').eq(6).val(filePath);
            $("#htcon1").show();
            $("#del1").show();
        }
    });
