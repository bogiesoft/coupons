...
// 					   print_r($user_grades);
					...
					<?php
					   ...
					   {
					       echo '<input type="checkbox" id="grade_'.$k.'" name="grade['.$k.']" value="'.$v['id'].'"';
					       if(!empty($_GET['grade'][$k]))
					           echo 'checked';
					       echo '><label for="grade_'.$k.'">'.$v['name'].'</label>';
					   }
...
// 					   var_dump(empty($user_groups));
					...
					<?php 
					   ...
					   {
					       echo '<input type="checkbox" id="group_'.$k.'" name="group['.$k.']" value="'.$v['id'].'"';
					       if(!empty($_GET['group'][$k]))
					           echo 'checked';
					       echo '><label for="group_'.$k.'">'.$v['name'].'</label>';
					   }
					?>
				</div>
				<?php }?>
			...
			<div class="user_list">
				...
				<div name="discuss" class="user_list_middle clearfix">
                    ...
                    <a href="<?php echo Yii::app()->createUrl('mCenter/user/userInfo',array('id'=>$v['id']))?>">
					<div class="td_item td_item_head">
						<div class="headIcon">
							<?php 
                                ...
                                ?>
							<img src="<?php echo $img?>" alter="<?php echo isset($v['nickname']) ? $v['nickname'] : '&nbsp;'?>" title="<?php echo $v['nickname'] ? $v['nickname'] : '&nbsp;'?>" style="cursor: pointer;" onclick="window.location.href='<?php echo Yii::app()->createUrl('mCenter/user/userDetail',array('id'=>$v['id']))?>'">
						</div>
						<div class="user_info">
						<?php
						      $vname = '';
						      ...
						      if($v['name'])
						          $vname = $v['name'];
						?>
							<span class="name" title="<?php echo $vname ?$vname : '&nbsp;'?>"  style="cursor: pointer;" onclick="window.location.href='<?php echo Yii::app()->createUrl('mCenter/user/userDetail',array('id'=>$v['id']))?>'"><?php echo $vname?$vname:'&nbsp;'?></span>
							<span>
								<em class="sex"><?php echo isset($v['sex']) ? $GLOBALS['__SEX'][$v['sex']] : ''?></em>
                                <?php if(!empty($v['birthday'])){?>
                                <em class="age">
                                    <?php echo (date('Y-m-d H:i:s')-$v['birthday']+1).'岁';?>
                                </em>
                                <?php }?>
                                </span>
					...
					<div class="td_item">
					<?php 
					   $current_user_orders = $user_store_order[$v['id']];
					   if(empty($current_user_orders))
					       echo "<span>&nbsp;</span><span>&nbsp;</span>";
					   else 
					       echo "<span>{$current_user_orders['store']['name']}<!--({$current_user_orders['store']['id']})--></span><span>{$current_user_orders['create_time']}</span>";
				    ?>
					</div>
					<div class="td_item">
					   <span>&nbsp;</span> <span>
					   <?php 
					       ...
					       if(!empty($current_user_orders))
					       {
					           foreach($current_user_orders as $vv)
					           {
					               if($vv->order_status==ORDER_STATUS_NORMAL)
					               {
					                   $shishou = $vv->order_paymoney - $vv->coupons_money - $vv->discount_money - $vv->merchant_discount_money;
					                   $consums += $shishou;
					               }
					           }
					       }
		...
		$(function() {
            $("#store_pop_btn").click(function(){
                ...
                layer.open({
                    ...
                    area : ['680px',"654px"],
                    content: '/mCenter/user/popStore'+param,
                });
            });
    });
    // 自动调整iframe的高度适应自身高度
