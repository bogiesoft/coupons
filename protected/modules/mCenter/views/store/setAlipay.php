	...
	<div class="paySet">
        <div class="top">
            ...
            <div class="menu clearfix">
                ...
                <span><a href = "<?php echo Yii::app() -> createUrl('mCenter/store/SetWechatPay',array('id' => $alipay -> id))?>">微信支付设置</a></span>
            </div> 
        </div>
...
$('.toggleBtn .switch').on('switchChange.bootstrapSwitch',function(event,state){
	if(state){
		...
		window.parent.autoResize('main');
		$.ajax({
			url : '<?php echo Yii::app()->createUrl('mCenter/store/SetAlipayOpen'); ?>',
...
$('input[name=use_pro]').change(function(){
	if($(this).is(':checked')) {
		$.ajax({
			url : '<?php echo Yii::app()->createUrl('mCenter/store/SetAlipayUse'); ?>',
			data : {state : 'true',id:<?php echo $alipay -> id?>},
			type : 'post',
			async : false,
			success : function(res){
