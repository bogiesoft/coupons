...
<body>
    ...
    <div class="kkfm_r_inner">
        ...
        <?php echo CHtml::beginForm() ?>
            <?php if ($model['sync_type'] == STORE_ALIPAY_SYNC_TYPE_SYNC) { ?>
            	<?php if ($model['verify_status'] == STORE_ALIPAY_SYNC_STATUS_AUDITING) {
            		$auth_status = 'authorizing';
            	}?>
            	<?php if ($model['verify_status'] == STORE_ALIPAY_SYNC_STATUS_REJECT) {
            		$auth_status = '';
            	}?>
            	<?php if ($model['verify_status'] == STORE_ALIPAY_SYNC_STATUS_PASS) {
            		$auth_status = 'authorized';
            	}?>
	            <div class="unauthorized <?php echo $auth_status; ?>">
	                <div class="unauthorizedTip">
	                    <h3>审核状态：<?php echo $GLOBALS['__STORE_ALIPAY_SYNC_STATUS'][$model['verify_status']]; ?></h3>
	                    <?php if ($model['verify_status'] == STORE_ALIPAY_SYNC_STATUS_REJECT) { ?>
	                    	<p>驳回原因：<?php echo $model['audit_desc']; ?></p>
	                    <?php }?>
	                    <p>时间：<time><?php echo $model['sync_time']; ?></time></p>
	                </div>
	            </div>
            <?php }?>
            <div class="basicInfo">
                ...
                <div class="filed">
                    <div class="syncInfo">
                        <input type="checkbox" id="checkboxN" onclick="setcheckboxN()" <?php echo $model['verify_status'] == STORE_ALIPAY_SYNC_STATUS_AUDITING || $model['verify_status'] == STORE_ALIPAY_SYNC_STATUS_PASS ? 'disabled' : ''; ?> <?php echo $model['sync_type'] == STORE_ALIPAY_SYNC_TYPE_SYNC ? 'checked' : '';?>>
                        <label for="checkboxN">口碑签约开店一体化</label>
                        <p>开店并同时签约口碑当面付，口碑泛行业可在支付宝手机客户端-口碑栏目中展示门店</p>
                    </div>
                    <div class="syncInfo">
                        <input type="checkbox" id="checkboxY" onclick="setcheckboxY()" <?php echo $model['verify_status'] == STORE_ALIPAY_SYNC_STATUS_AUDITING || $model['verify_status'] == STORE_ALIPAY_SYNC_STATUS_PASS ? 'disabled' : ''; ?> <?php echo $model['sync_type'] == STORE_ALIPAY_SYNC_TYPE_RELATION ? 'checked' : '';?>>
...
上传照片中需要包含一张门头照片，两张内景照，如无门头照片将会审核失败
                ...
                <div class="upImg" id="upImg_mdsjt">
					...
					if (!empty($audit_image)) {
						foreach ($audit_image as $img) { ?>
							<div class="img">
                                <em class="close" onclick="delImgMdsjt(this)">
                                </em>
                ...
                <div class="upImg">
                	...
                	<?php echo CHtml::hiddenField('Store[brand_logo]', $model['brand_logo'])?>
                </div>
                </span>
                </div>
                <div class="filed">
            ...
            <div class="basicInfo" id="lastbasicInfo">
                ...
                <input type="button" class="btn_com_gray"  value="取消" onclick="window.history.back()">
             </span>
                </div>
            </div>
