	...
	<div class="paySet">
        <div class="top">
            ...
            <div class="menu clearfix">
            	<span><a href="<?php echo Yii::app() -> createUrl('mCenter/store/aliPayMent',array('id' => $wechat -> id));?>">支付宝当面付设置</a></span>
...
function t_isempty(){
<?php if(empty($wechat -> t_wx_appsecret) && empty($wechat -> t_wx_mchid)){?>
	$('#save').show();
	$('#edit').hide();
	$('#t_appid_text').hide();
...
$('.toggleBtn .switch').on('switchChange.bootstrapSwitch',function(event,state){
	if(state){
		...
		$.ajax({
			url : '<?php echo Yii::app()->createUrl('mCenter/store/SetManagementWechatOpen'); ?>',
...
$('input[name=use_pro]').change(function(){
	if($(this).is(':checked')) {
		$.ajax({
			url : '<?php echo Yii::app()->createUrl('mCenter/store/SetManagementWechatUse'); ?>',
			data : {state : 'true',id:<?php echo $wechat -> id?>},
			type : 'post',
			async : false,
			success : function(res){
...
$(function () {
    $('#key_upload').uploadify({
        ...
        onUploadSuccess: function (file, data, response) {//一个文件上传成功后的响应事件处理
            ...
            $.ajax({
                url:'<?php echo Yii::app()->createUrl('mCenter/store/uploadSuc');?>',
                cache:false,
