...
<div class="kkfm_r_inner">
    <div class="top">
        ...
        <div class="cz">
            <a href="<?php echo $goUrl; ?>" class="btn_com_gray">返回</a>
        </div>
    </div>
    <div class="orderDetail">
        <div class="search clearfix">
            <div class="step">
                <ul class="<?php if ($list['pay_status'] == ORDER_STATUS_PAID && $list['order_status'] == ORDER_STATUS_WAITFORDELIVER) {
                        echo 'step02';
                    } elseif ($list['order_status'] == ORDER_STATUS_DELIVER) {
                        echo 'step03';
                    } elseif ($list['order_status'] == ORDER_STATUS_ACCEPT) {
                        echo 'step04';
                    } else {
                        echo 'step01';
                    } ?>">
                    ...
                    <li>
                        <span class="stepName color">买家下单</span>
                        <span class="stepTime"><?php echo $list['create_time']; ?></span>
                    </li>
                    <li>
                        <span class="stepName <?php echo $list['pay_status'] == ORDER_STATUS_PAID ? 'color' : ''; ?>">买家付款</span>
                        <span class="stepTime"><?php echo $list['pay_time']; ?></span>
                    </li>
                    <li>
                        <span class="stepName <?php echo $list['order_status'] == ORDER_STATUS_DELIVER || $list['order_status'] == ORDER_STATUS_ACCEPT ? 'color' : ''; ?>">商家出票</span>
                        <span class="stepTime"><?php echo $list['send_time']; ?></span>
                    </li>
                    <li>
                        <span class="stepName <?php echo $list['order_status'] == ORDER_STATUS_ACCEPT ? 'color' : ''; ?>">完成交易</span>
        ...
        <div class="orderWarp clearfix">
            <div class="orderL">
                ...
                <div class="filed">
                    <span class="label">订单码号：</span>
                    <span class="text"><?php echo $list['thrid_party_code']; ?></span>
                </div>
                <div class="filed">
                    <span class="label">付款方式：</span>
                    <span class="text"><?php echo isset($list['pay_channel']) ? $GLOBALS['ORDER_PAY_CHANNEL'][$list['pay_channel']] : ''; ?></span>
                </div>
                <?php $address_arr = explode(',', $list['address'])?>
                <div class="filed">
                    <span class="label">买家：</span>
                    <span class="text"><?php echo $address_arr[0].'('.$address_arr[1].')'; ?></span>
                </div>
                <br>
                ...
                <div class="filed">
                    ...
                    <span class="text">电子票自动发货</span>
                </div>
                <div class="filed">
                    <span class="label">买家留言：</span>
                    <span class="text"><?php echo $list['remark']; ?></span>
                </div>
            </div>
            <div class="orderR">
                <div class="title">
						<span class="status">订单状态：<?php if ($list['pay_status'] == ORDER_STATUS_UNPAID) {
                                echo '买家已下单,等待买家付款';
                            } elseif ($list['pay_status'] == ORDER_STATUS_PAID && $list['order_status'] == ORDER_STATUS_WAITFORDELIVER && !$refund_status) {
                                echo '买家已付款，等待卖家发货';
                            } elseif ($list['order_status'] == ORDER_STATUS_DELIVER) {
                                echo '卖家已发货';
                            } elseif ($list['pay_status'] == ORDER_STATUS_PAID && $list['order_status'] == ORDER_STATUS_ACCEPT) {
                                echo '完成交易';
                            } else {
                                echo "商品正在退款或已经取消不能发货";
                            } ?></span>
                    <?php if ($list['pay_status'] == ORDER_STATUS_PAID && $list['order_status'] == ORDER_STATUS_WAITFORDELIVER) { ?>
                        <span>
                            <?php if (!$refund_status) { ?>买家已付款，请尽快发货，否则买家有权申请退款<?php } else {
                                echo "商品正在退款或已经取消不能发货";
                            } ?>
                        </span>
                    <?php } ?>
                ...
                <div class="filed">
                    ...
                    <span class="text"><?php echo $list['seller_remark']; ?></span>
                </div>
            </div>
        </div>
        <!--end orderWarp-->
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0"
                    ...
                    <?php for ($i = 0; $i < count($list['order_sku']); $i++) { ?>
                        <tr class="orderCon">
                            <td>
                                <a href="#">
                                    <div class="list-img">
                                        <img src="<?php $img = explode(';', $list['order_sku'][$i]['product_img']);
                                        echo IMG_GJ_LIST . $img[0]; ?>"/>
                                    </div>
                                    <div class="list-name"><?php echo $list['order_sku'][$i]['product_name']; ?></div>
                                    <div class="list-info"><?php echo $list['order_sku'][$i]['sku_name']; ?></div>
                                </a>
                            </td>
                            <td style="font-size: 16px">¥ <?php echo $list['order_sku'][$i]['price']; ?></td>
                            <td style="font-size: 16px"><?php echo $list['order_sku'][$i]['num']; ?></td>
                            <td style="font-size: 16px">¥ <?php echo ($list['order_sku'][$i]['price']) * ($list['order_sku'][$i]['num']); ?></td>
                            <?php if ($i == 0) { ?>
                                <td rowspan="<?php echo count($list['order_sku']); ?>" style="font-size: 16px" class="border"><?php echo $list['order_sku'][$i]['consume_num']; ?> 次</td>
                            <?php } ?>
                        </tr>
                    <?php } ?>
                <?php } ?>
                ...
                <tr class="orderHead">
                    <td colspan="6" align="right">
                        <span>应收总价：<em><?php echo number_format($list['real_pay'] + $list['freight_money'], 2, '.', ''); ?></em></span>
                    </td>
                </tr>
                <tr>
...
<div class="popWrap" id="pop" style="width: 655px; display: none">
    <?php echo CHtml::beginForm(Yii::app()->createUrl('mCenter/dshopOrder/editAddress', array('order_id' => $list['id'])), 'get', array('id' => 'editForm')); ?>
    <div class="pop_con pAddress">
        <div class="title">
            <span class="name">修改收货地址</span>
            <span class="cz">
                <a href="javascript:;" onclick="closeWin()">X</a>
            </span>
        </div>
        <div class="pop_content">
            ...
            <div class="filed">
                <span class="label"></span> <span class="text">
                    <input type="button" value="确定" class="btn_com_blue" style="margin-right: 20px" onclick="save()">
...
<div class="popWrap" id="seller_remark_pop" style="width: 550px; display: none">
    <?php echo CHtml::beginForm(Yii::app()->createUrl('mCenter/dshopOrder/editSellerRemark', array('order_id' => $list['id'], 'type' => 'detail')), 'get', array('id' => 'editRemarkForm')); ?>
    <div class="pop_con pop_remark pAddress">
        <div class="title">
            <span class="name">修改卖家备注</span> <span class="cz">
                <a href="javascript:;" onclick="closeSellRemarkWin()">X</a>
            </span>
        </div>
        <div class="pop_content">
            <div class="filed">
                <span class="label stick">备注：</span>
                <span class="text"><?php echo CHtml::textArea('seller_remark', $list['seller_remark'], array('placeholder' => '请填写备注，字数在250字内')); ?></span>
...
<div class="popWrap" id="send_pop" style="width: 600px; display: none">
    <div class="pop_con">
        ...
        <?php echo CHtml::beginForm(Yii::app()->createUrl('mCenter/dshopOrder/send', array('order_id' => $list['id'], 'type' => 'detail')), 'get', array('id' => 'sendForm')); ?>
                ...
                <?php if (!empty($list['order_sku'])) { ?><?php $length = 0; ?>
                    <?php for ($i = 0; $i < count($list['order_sku']); $i++) { ?>
                        <?php if ($list['order_sku'][$i]['if_send'] == IF_SEND_NO && ($list['order_sku'][$i]['status'] == ORDER_SKU_STATUS_NORMAL || $list['order_sku'][$i]['status'] == ORDER_REFUND_STATUS_APPLY_REFUND_RETURN)) { ?>
                            ...
                            <tr class="list-title">
                                <td>
                                    <input class="sku" type="checkbox" name='order_sku[]'
                                           value="<?php echo $list['order_sku'][$i]['id']; ?>">
                                </td>
                                <td align="left">
                                    <a href="#" class="blue"><?php echo $list['order_sku'][$i]['product_name']; ?></a>
                                </td>
