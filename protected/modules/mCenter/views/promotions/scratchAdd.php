...
<!-- 	             </span> -->
	         ...
	         <div class="filed">
	             <span class="label">四等奖奖品设置</span>
<!-- 	             <span class="text"> -->
	             	<?php //echo CHtml::textField('Activity[first_prize]', $model['first_prize'])?>
<!-- 	             </span> -->
	             	<span class="text">
		             	<?php echo CHtml::hiddenField('Activity[fourth_prize]', $model['fourth_prize'])?>
		             	<a href="javascript:;" onclick="addHeaderHref(<?php echo PRIZE_TYPE_FORTH?>)" id="addFourthHref"><?php echo !empty($title_arr[PRIZE_TYPE_FORTH]['title']) ? $title_arr[PRIZE_TYPE_FORTH]['title'] : "添加卡券链接" ?></a>
		            </span>
	         </div>
	         <div class="filed">
	             <span class="label">四等奖奖品数量</span>
	             <span class="text">
	             	<?php echo CHtml::textField('Activity[fourth_prize_num]', $model['fourth_prize_num'])?>
	             </span>
	         </div>
	         <div class="filed">
	             <span class="label">四等奖奖品概率</span>
	             <span class="text">
	             	<?php echo CHtml::textField('Activity[fourth_prize_probability]', $model['fourth_prize_probability'])?>
	             </span>
	             <span class="text1">% *百分比，在0-100之间, 支持小数点</span>
	         </div>
	         <div class="filed">
	             <span class="label">五等奖奖品设置</span>
<!-- 	             <span class="text"> -->
	             	<?php //echo CHtml::textField('Activity[first_prize]', $model['first_prize'])?>
<!-- 	             </span> -->
	             	<span class="text">
		             	<?php echo CHtml::hiddenField('Activity[fifth_prize]', $model['fifth_prize'])?>
		             	<a href="javascript:;" onclick="addHeaderHref(<?php echo PRIZE_TYPE_FIFTH?>)" id="addFifthHref"><?php echo !empty($title_arr[PRIZE_TYPE_FIFTH]['title']) ? $title_arr[PRIZE_TYPE_FIFTH]['title'] : "添加卡券链接" ?></a>
		            </span>
	         </div>
	         <div class="filed">
	             <span class="label">五等奖奖品数量</span>
	             <span class="text">
	             	<?php echo CHtml::textField('Activity[fifth_prize_num]', $model['fifth_prize_num'])?>
	             </span>
	         </div>
	         <div class="filed">
	             <span class="label">五等奖奖品概率</span>
	             <span class="text">
	             	<?php echo CHtml::textField('Activity[fifth_prize_probability]', $model['fifth_prize_probability'])?>
	             </span>
	             <span class="text1">% *百分比，在0-100之间, 支持小数点</span>
	         </div>
	         <div class="filed">
...
// 	              	 height: '446px',
	...
	function addHrefChoose(prize_type, id, name)
	{
		if(prize_type == <?php echo PRIZE_TYPE_FIRST?>){
			...
			art.dialog.get("dialog_notice").close(); //关闭弹出框
		}else if(prize_type == <?php echo PRIZE_TYPE_FORTH?>){
			$('#Activity_fourth_prize').val(id);
			$('#addFourthHref').text(name);
			art.dialog.get("dialog_notice").close(); //关闭弹出框
		}else if(prize_type == <?php echo PRIZE_TYPE_FIFTH?>){
			$('#Activity_fifth_prize').val(id);
			$('#addFifthHref').text(name);
			art.dialog.get("dialog_notice").close(); //关闭弹出框
		}
	}
