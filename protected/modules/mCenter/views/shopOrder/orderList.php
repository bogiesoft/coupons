...
<div class="kkfm_r_inner">
     ...
     <div class="shop_menu">
    	<ul>
        	<li class="cur"><a href="<?php echo Yii::app()->createUrl('mCenter/shopOrder/orderList') ?>">全部订单</a></li>
        	 <li>
        		<a href="<?php echo Yii::app()->createUrl('mCenter/shopActivist/ActivistList') ?>">维权订单</a>
        	</li>
        </ul>
     </div>
     <div class="orderList">
     ...
     <?php echo CHtml::beginForm('','get'); ?>
     	<div class="search clearfix">
            ...
            <div class="filed">
                ...
                <span class="text">
                	<?php echo CHtml::dropDownList('Order[order_type]', /*isset($_GET['Order']['order_type'])?$_GET['Order']['order_type']:*/'', $GLOBALS['ORDER_TYPE'],array('prompt'=>'请选择')); ?>
                </span>
            </div>
            <div class="filed">
            	<span class="label">订单状态</span>
                <span class="text">
                   	<?php echo CHtml::dropDownList('order_status', isset($_GET['order_status'])?$_GET['order_status'] : '', $order_status,array('prompt'=>'请选择')); ?>
                </span>
            </div>
            <div class="clear"></div>
            <div class="filed">
...
<!--            </div>-->
            ...
            <div class="clear"></div>
             <div class="filed ">
                ...
                <span class="text">
                	...
                	<a href="<?php echo Yii::app()->createUrl('mCenter/shopOrder/exportExcel',
        ...
        <div class="orderWarp">
            <div class="status-nav clearfix">
                <ul>
                    <li class="<?php echo (!empty($_GET['order_status'])||isset($_GET['pay_status']))?'':'cur'; ?>"><a href="<?php echo Yii::app()->createUrl('mCenter/shopOrder/orderList'); ?>">全部</a></li>
                    <li class="<?php if(isset($_GET['pay_status'])){echo ($_GET['pay_status']==ORDER_STATUS_UNPAID && !isset($_GET['order_status']))?'cur':'';} ?>"><a href="<?php echo Yii::app()->createUrl('mCenter/shopOrder/orderList',array('pay_status'=>ORDER_STATUS_UNPAID)); ?>">待付款<?php if($noPayCount > 0){echo '('.$noPayCount.')';} ?></a></li>
                    <li class="<?php if(isset($_GET['order_status'])){echo $_GET['order_status']==ORDER_STATUS_WAITFORDELIVER?'cur':'';} ?>"><a href="<?php echo Yii::app()->createUrl('mCenter/shopOrder/orderList',array('order_status'=>ORDER_STATUS_WAITFORDELIVER)); ?>">待发货<?php if($waitCount > 0){echo '('.$waitCount.')';} ?></a></li>
                    <li class="<?php if(isset($_GET['order_status'])){echo $_GET['order_status']==ORDER_STATUS_DELIVER?'cur':'';} ?>"><a href="<?php echo Yii::app()->createUrl('mCenter/shopOrder/orderList',array('order_status'=>ORDER_STATUS_DELIVER)); ?>">已发货<?php if($deliverCount > 0){echo '('.$deliverCount.')';} ?></a></li>
                   <!-- 
                    <li class="<?php //if(isset($_GET['order_status'])){echo $_GET['order_status']==ORDER_STATUS_HANDLE_REFUND?'cur':'';} ?>"><a href="<?php //echo Yii::app()->createUrl('mCenter/shopOrder/orderList',array('order_status'=>ORDER_STATUS_HANDLE_REFUND)); ?>">退款中</a></li>
                    <li class="<?php //if(isset($_GET['order_status'])){echo $_GET['order_status']==ORDER_STATUS_REFUND?'cur':'';} ?>"><a href="<?php //echo Yii::app()->createUrl('mCenter/shopOrder/orderList',array('order_status'=>ORDER_STATUS_REFUND)); ?>">已退款</a></li>
 					--> 
					<li class="<?php if(isset($_GET['order_status'])){echo $_GET['order_status']==ORDER_STATUS_ACCEPT?'cur':'';} ?>"><a href="<?php echo Yii::app()->createUrl('mCenter/shopOrder/orderList',array('order_status'=>ORDER_STATUS_ACCEPT)); ?>">已收货<?php if($acceptCount > 0){echo '('.$acceptCount.')';} ?></a></li>
                    <li class="last <?php if(isset($_GET['order_status'])){echo $_GET['order_status']==ORDER_STATUS_CANCEL?'cur':'';} ?>"><a href="<?php echo Yii::app()->createUrl('mCenter/shopOrder/orderList',array('order_status'=>ORDER_STATUS_CANCEL)); ?>">已取消<?php if($cancelCount > 0){echo '('.$cancelCount.')';} ?></a></li>
                </ul>
                <hr>
            </div>
    			...
    			<?php if(!empty($v['order_sku'])){?>
                  <?php for($i=0;$i<count($v['order_sku']);$i++){ ?>
                  	<tr class="orderCon">
                        ...
                        <td class="border" width="90"><!-- rowspan="--><?php /*echo count($v['order_sku']); */?><?php if($v['order_status'] == ORDER_STATUS_REFUND){echo '用户提交维权';} ?></td>
                        <?php if($i==0){?>
                        ...
                        <td rowspan="<?php echo count($v['order_sku']); ?>"><?php echo number_format($v['real_pay'],2,'.',''); ?><span></span></td>
                  	    <?php }?>
                      </tr>
                  <?php }?>
                 <?php }?>
...
<div class="popWrap" id="seller_remark_pop" style="width:550px;display:none;position: fixed">
 <?php echo CHtml::beginForm(Yii::app()->createUrl('mCenter/shopOrder/editSellerRemark',array('type'=>'list')),'get',array('id'=>'editRemarkForm')); ?>
...
<div class="popWrap" id="send_pop" style="width:600px; display:none">
    <div class="pop_con">
		...
		<?php echo CHtml::beginForm(Yii::app()->createUrl('mCenter/shopOrder/send',array('type'=>'list')),'get',array('id'=>'sendForm')); ?>
...
function saveRemark()
{
	...
	function ordersend(order_id,this_btn){
		...
		$.ajax({
			url : '<?php echo Yii::app()->createUrl('mCenter/shopOrder/getAjaxDataForSend'); ?>',
			data : {order_id : order_id},
			type : 'post',
			async: false,
			success : function(data){
