 ...
 *
 */
class DshopOrderController extends mCenterController
{
    ...
    public function actionOrderDetail($order_id)
    {
        ...
        $refund_stutus = false;
        for ($i = 0; $i < count($list['order_sku']); $i++) {
    ...
    public function actionEditAddress($order_id)
    {
        ...
        if (!$flag) {
            ...
            if ($result['status'] == ERROR_NONE) {
                $this->redirect(Yii::app()->createUrl('mCenter/dshopOrder/orderDetail', array('order_id' => $order_id)));
            }
        }
    }
    ...
    public function actionEditSellerRemark()
    {
        ...
        if ($result['status'] == ERROR_NONE) {
            if ($type == 'detail') { //如果是在详情界面修改   则跳到详情界面
                ...
                $this->redirect(Yii::app()->createUrl('mCenter/dshopOrder/orderList'));
            } elseif ($type == 'activistList') { //如果是在订单维权列表界面修改   则跳到订单维权列表界面
                $this->redirect(Yii::app()->createUrl('mCenter/dshopActivist/activistList'));
            }
        }
    }
