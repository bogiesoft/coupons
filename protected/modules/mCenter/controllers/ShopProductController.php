...
class ShopProductController extends mCenterController{
	...
	public function actionAddProductOfCategory()
	{
		...
		if(!empty($_POST['Product']) && isset($_POST['Product'])){
			...
			$freight_money = isset($post['pro_total_inventory']) ? $post['pro_total_inventory'] : "";
			...
			$save_pro_res = $shop->saveProduct($id, $merchant_id, $category_id, $group_id, $standard, $page_id, $type, $name, $price, $img_arr, $freight_type, $freight_money, $freight_id, $if_invoice, $sku_arr, $limit_num, $brief_introduction, $detailed_introduction, $if_show_num, $stock_num);
...
// 			$num = $post['num'];
		...
		$this->render('addProductOfCategory', array('category_one'=>$category_one, 'category_two'=>$category_two, 'standard_model' => $standard_model, 'standard_id' => $standard_id, 'group'=>$group, 'freight' => $freight));
	}
	...
	public function actionEditProductOfCategory()
	{
		...
		if(!empty($_POST['Product']) && isset($_POST['Product'])){
			...
			if ($save_pro_res['status'] == ERROR_NONE) {
				...
				$list['page_id'] = $post['standard_model'];
				if (!empty($post['pro'])){
					$num = 0;
