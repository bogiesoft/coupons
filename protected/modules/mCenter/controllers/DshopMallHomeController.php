 ...
 * 2015/09/22
 */
class DshopMallHomeController extends mCenterController{
...
// 		}
    ...
    public function actionSaveShopMall()
    {
        ...
        if(!empty($_POST['data_order']))
        {
            ...
            $qcord_arr=$_POST['data_qcord_arr'];
            $catalog_arr=$_POST['data_catalog_arr'];
            ...
            for($order_i=0;$order_i<count($order);$order_i++)
            {
                ...
                else if($order_str[0]=='qcord')
                {
                    //商品分组
                    $json[$order_i]['name']='qcord';
                    $json[$order_i]['url']=$qcord_arr[intval($order_str[1])];
                    $json[$order_i]['href']='null';
                }
                else if($order_str[0]=='catalog')
                {
                    //商品分组
                    $json[$order_i]['name']='catalog';
                    $json[$order_i]['url']=$catalog_arr[intval($order_str[1])];
                    $json[$order_i]['href']='null';
                }
            }
    ...
    public function actionShopMallIndex()
    {
        ...
        {
            if($banner[$i]['name']=='shop_search')
            {
                $shop_search[$shop_search_count]=$banner[$i];
                $shop_search_count++;
            }
            else if($banner[$i]['name']=='img_nav')
            {
                $img_nav[$img_nav_count]=$banner[$i];
                $img_nav_count++;
            }
            else if($banner[$i]['name']=='img_adv')
            {
                $img_adv[$img_adv_count]=$banner[$i];
                $img_adv_count++;
            }
            else if($banner[$i]['name']=='shop_carousel')
            {
                $shop_carousel[$shop_carousel_count]=$banner[$i];
                $shop_carousel_count++;
            }
            else if($banner[$i]['name']=='shop_group')
            {
                $shop_group[$shop_group_count]=$banner[$i];
                $shop_group_count++;
            }
            else if($banner[$i]['name']=='qcord')
            {
                $qcord[$qcord_count]=$banner[$i];
                $qcord_count++;
            }
            else if($banner[$i]['name']=='catalog')
            {
                $catalog[$catalog_count]=$banner[$i];
                $catalog_count++;
            }
        }
