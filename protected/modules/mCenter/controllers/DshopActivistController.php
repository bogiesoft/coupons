...
class DshopActivistController extends mCenterController{
	...
	public function actionActivistList()
	{
		...
		$activistC = new DActivistC();
		$result = $activistC -> getActivistList($merchant_id);
	...
	public function actionAgreeRefund($order_id,$order_sku_id,$order_status)
	{
      	...
      	$url = Yii::app()->createUrl('mCenter/dshopActivist/ActivistDetail',array(
      			'order_id'=>$order_id,
      			'order_sku_id'=>$order_sku_id,
      			'order_status'=>$order_sku_id
      	));
      	//先判断该订单是否已经退款，已经退款就不调用支付宝接口
      	if($order_status != REQUEST_ORDER_STATUS_REFUND){
//        		//支付宝退款
//       		if($list['pay_channel'] == ORDER_PAY_CHANNEL_ALIPAY){
//            		echo("<script>parent.location.href='".Yii::app() -> createUrl('mCenter/dshopActivist/WappayUrl',array('sellerId'=>$sellerId,'trade_no'=>$list['trade_no'],'order_paymoney'=>$list['order_paymoney'],'remark'=>$list['remark'],'key'=>$key,'refund_order_no'=>$list['refund_order_no'],'seller_email'=>$seller_email))."'</script>");
//       		}
       		//微信退款
      		if($list['pay_channel'] == ORDER_PAY_CHANNEL_WXPAY){
          		$wappayC = new WappayC();
          		$result = json_decode($wappayC ->MallRefund($order_id, $order_sku_id, $order_status),true);
          		if($result['status']==ERROR_NONE)
          		{
              		$order=new DActivistC();
             		$result=json_decode($order->AgreeTake($order_id,$order_sku_id,$order_status),true);
              		if($result['status']==ERROR_NONE)
              		{                                
                  		$this->redirect(Yii::app()->createUrl('mCenter/dshopActivist/activistDetail',array(
                  				'order_id'=>$order_id,
                  				'order_sku_id'=>$order_sku_id,
	...
	public function actionRefuseRefund()
	{
        ...
        if(isset($_POST))
        {
            ...
            $result = json_decode($result,true);
            if($result['status'] == ERROR_NONE){
                $this->redirect(Yii::app()->createUrl('mCenter/dshopActivist/activistDetail',array('order_id'=>$order_id,'order_sku_id'=>$order_sku_id,'order_status'=>$order_status)));
            }
        }
	...
	public function actionAddRefundAddress()
	{
		...
		$result = json_decode($result,true);
		if($result['status'] == ERROR_NONE){
			$this->redirect(Yii::app()->createUrl('mCenter/dshopActivist/activistDetail',array('order_id'=>$refuse_orderid,'order_sku_id'=>$order_sku_id,'order_status'=>$refund_status)));
		}
	}
            ...
            if($order_status != REQUEST_ORDER_STATUS_REFUND){
                ...
                if($list['pay_channel'] == ORDER_PAY_CHANNEL_WXPAY){
                    ...
                    if($result['status']==ERROR_NONE)
                    {
                        ...
                        {
                            $wappay = new WappayC();
                            $wappay ->wappayNotifyUrl($order_id);
                            $this->redirect(Yii::app()->createUrl('mCenter/dshopActivist/activistDetail',array('order_id'=>$order_id,'order_sku_id'=>$order_sku_id,'order_status'=>$order_status)));
                        }                            
                    } else {
