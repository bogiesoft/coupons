...
<nav class="nav m-cp-nav">
    <span class="nav-link" onclick="window.location.href='<?php echo $this->createUrl('storeStatisticsDay')?>'">日汇总</span>
    <span class="nav-link active" onclick="window.location.href='<?php echo $this->createUrl('storeStatisticsMonth')?>'">月汇总</span></nav>
<div class="g-container">
    <div class="m-box store-trade-search">
        <div class="form-inline m-bg-search">
        <?php echo CHtml::beginForm($this->createUrl('storeStatisticsMonth'),'get',array('id'=>"search_form")); ?>
            <div class="m-bg-search-item">
                <div class="form-group">
                    <label>交易时间：</label>
                    <?php echo CHtml::textField('Time', isset($_GET['Time']) ? $_GET['Time'] : date('Y/m',strtotime('-12 month')).' - '.date('Y/m',strtotime('-1 month')), array('class'=>"form-control fc-s-sm ml10",'readonly'=>"readonly"))?>
                </div>
    ...
    <a href="<?php echo $this->createUrl('exportMonthsExcel',array('get'=>empty($_GET)?array():$_GET));?>" class="download-icon download fr"></a>
        <div class="m-box trade-data">
            ...
            <div class="trade-data-item">
                <div class="m-row trade-summary-data">
                    <div class="m-col-md text-right">
                        <p class="text-left pl20 fl"><?php echo date('Y年',strtotime($k))?><br><?php echo date('n月',strtotime($k))?></p>
                    ...
                    ?>
                    <div class="m-col-md m-col-md-1 text-right">
                        <a href="<?php echo $this->createUrl('exportMonthExcel',array('get'=>$get,))?>">下载月账单</a>
                    </div>
    ...
    <p class="text-gray mt10">月汇总会在次月生成</p>
    <!-- 选择门店弹出 -->
