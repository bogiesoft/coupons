...
class QRcodeCreator {
	...
	const folder_path = QRCODE_SYSTEM_PATH; //文件保存路径
	...
	*/
	 ...
	 * @param unknown $logo_path
	 * @param unknown $size
	 * @param unknown $margin
	 * @param unknown $style
	 * @return string
	 */
	public static function createQrcode($data, $outfile, $logo_path, $size, $margin, $style) {
		//引入phpqrcode库文件
		...
		if (!$outfile) {
			$print = true;
			$outfile = self::folder_path.uniqid().mt_rand(10000, 99999).'.png'; //临时文件存储路径
		}
		//创建一个二维码文件: 二维码数据内容，输出图片文件，纠错等级，图片大小，空白区域大小，是否打印保存，样式
		QRcode::png($data, $outfile, QR_ECLEVEL_H, $size, $margin, false, $style);
	 ...
	 */
	public static function createWqQrcode($data, $string, $branch_string, $bg_path, $logo_path) {
		//开始创建二维码
		$outfile = self::folder_path.uniqid().mt_rand(10000, 99999).'.png'; //临时文件存储路径
		self::createQrcode($data, $outfile, $logo_path, 13.5, 0, null);
		...
		//文字信息合并到大图
		$color1 = imagecolorallocate($img, 255, 255, 255); //文字颜色
		$font_width = $img_width; //文字宽度，等于图片宽度
// 		$text = QRcodeCreator::autowrap(80, 0, dirname(__FILE__).'/boldfont.ttc', $string, $font_width);
// 		$text = explode('_', $text);
// 		$tmp = $text['0'];
		//获取文字图片的宽度
		$font_img_width1 = QRcodeCreator::getFontImageWidth(56, 0, dirname(__FILE__).'/HYQiHei-55S.otf', $string);
		$font_img_width2 = QRcodeCreator::getFontImageWidth(32, 0, dirname(__FILE__).'/HYQiHei-55S.otf', $branch_string);
		//计算x坐标
		...
		imagettftext($img, 55, 0, $x1, $y1, $color1, dirname(__FILE__).'/HYQiHei-55S.otf', $string); //主门店名
		if (!empty($branch_string)) {
			imagettftext($img, 32, 0, $x2, $y2, $color1, dirname(__FILE__).'/HYQiHei-55S.otf', $branch_string); //分店名
		}
...
// 		$h1 = imagesy($qr);
		...
		*/
		...
		//输出图片到服务器，保存两张图片，一张预览图（压缩处理），一张原图（无压缩）
		$preview = substr($outfile, 0, strlen($outfile) - 4).'_view.jpg';
		imagejpeg($img, $preview, 20);
	 ...
	 * @param unknown $logo_path		logo路径
	 */
	public static function createAliQrcode($data, $string, $branch_string, $bg_path, $logo_path) {
		//开始创建二维码
		$outfile = self::folder_path.uniqid().mt_rand(10000, 99999).'.png'; //临时文件存储路径
		self::createQrcode($data, $outfile, $logo_path, 15.2, 0, array('innerColor' => array(0, 170, 239)));
		...
		//imagefill($img, 0, 0, $color); //颜色填充
		//文字信息合并到大图
		$color1 = imagecolorallocate($img, 0, 0, 0); //文字颜色
		$font_width = $img_width; //文字宽度，等于图片宽度
		// 		$text = QRcodeCreator::autowrap(80, 0, dirname(__FILE__).'/boldfont.ttc', $string, $font_width);
		// 		$text = explode('_', $text);
		// 		$tmp = $text['0'];
		//获取文字图片的宽度
		$font_img_width1 = QRcodeCreator::getFontImageWidth(36, 0, dirname(__FILE__).'/msyhbd.ttf', $string);
		$font_img_width2 = QRcodeCreator::getFontImageWidth(36, 0, dirname(__FILE__).'/msyhbd.ttf', $branch_string);
		//计算x坐标
		$x1 = ($img_width - $font_img_width1 + 10) / 2;
		$x2 = ($img_width - $font_img_width2 + 15) / 2.05;
		//设置y坐标
		$y1 = 2045;
		$y2 = $y1 + 62;
		//图片，字体大小，角度，x坐标，y坐标，颜色，字体文件，字符串内容
		imagettftext($img, 36, 0, $x1, $y1, $color1, dirname(__FILE__).'/msyhbd.ttf', $string); //主门店名
		if (!empty($branch_string)) {
			imagettftext($img, 36, 0, $x2, $y2, $color1, dirname(__FILE__).'/msyhbd.ttf', $branch_string); //分店名
		}
