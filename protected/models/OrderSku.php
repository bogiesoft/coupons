...
class OrderSku extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('order_id, sku_id, num, consume_num, status, flag, if_send', 'numerical', 'integerOnly'=>true),
			array('price', 'numerical'),
			array('product_name', 'length', 'max'=>100),
			array('create_time, last_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, order_id, sku_id, product_name, num, consume_num, price, create_time, status, flag, last_time, if_send', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('consume_num',$this->consume_num);
