...
class MerchantC extends mainClass{
	public $page = null;
	//查询商户列表
	/*
	 * $name 商户名
...
// 			}
         ...
         * $remark 驳回原因
         * */
        public function WqrejectMerchant($merchantId,$remark){
            $merchant = Merchant::model() -> find('id = :id and flag = :flag',array(
                ':id' => $merchantId,
                ':flag' => FLAG_NO
            ));
            if($merchant){
                ...
                try {
                    if($merchant -> wq_m_verify_status == MERCHANT_VERIFY_STATUS_WAIT){
                        $merchant -> wq_m_verify_status = MERCHANT_VERIFY_STATUS_REJECTIT;
                        if(isset($remark) && !empty($remark)){
                            $merchant -> wq_m_reject_remark = $remark;
                        }
                        if($merchant -> update()){
                            $transaction->commit();
                            $result['status'] = ERROR_NONE;
...
//                            $merchant->developer_public_key = $publickey;
	...
	public function editMerchantPwd($merchantId,$oldPwd,$pwd){
		...
		if (Yii::app()->session['role'] == WQ_ROLE_MANAGER){
		    ...
		    $criteria->params[':merchant_id'] = $merchantId;
		    $criteria->addCondition('id = :id');
		    $criteria->params[':id'] = $manager_id;
		    $criteria->addCondition('flag = :flag');
		    $criteria->params[':flag'] = FLAG_NO;
		    $criteria->addCondition('pwd = :pwd');
		    $criteria->params[':pwd'] = $oldPwd;
		    $model = Manager::model()->find($criteria);
		}else{
		    $criteria = new CDbCriteria();
		    $criteria->addCondition('id = :id');
		    $criteria->params[':id'] = $merchantId;
...
//     		print_r($dataLine);exit;
    ...
    public function getStoreArrW($merchant_id)
    {
    	...
    	foreach ($store as $k => $v) {
    		$stores['$k']['name'] = $v -> name;
    		$stores['$k']['branch_name'] = $v -> branch_name;
    	}
    	return $stores;
    }
...
/***************************************问卷调查开始***********************************************/
    ...
    /******************************************玩券管家******************************************/
    //获取商户玩券管家信息
    public function getMerchantWqinfo($merchant_id,$role = '',$manager_id = ''){
        $result = array();
        try {
            $merchant = Merchant::model() -> findByPk($merchant_id);
            $data = array();
