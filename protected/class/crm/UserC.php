...
//  				var_dump($diff1);
	 ...
	 * 改变会员的等级
	 */
	public function changeUserGrade($merchant_id) {
		$GradeDraft = json_decode($this -> getSetUserGradeDraft($merchant_id),true);
		$rule_type = $GradeDraft['datas']['list']['rule_type'];
		...
		$grade = UserGrade::model()->findAll($criteria);
		
		$users = User::model()->findAll('merchant_id=:merchant_id and flag=:flag',array(
				':merchant_id'=>$merchant_id,
				':flag'=>FLAG_NO
		));
		//按累计积分
		if($rule_type == 1){
			foreach ($users as $key=>$val){
				$userpoints = 0;
				$points = UserPointsdetail::model()->findAll('user_id=:user_id and balance_of_payments=:balance_of_payments',array(
						':user_id'=>$val['id'],
						':balance_of_payments'=>BALANCE_OF_PAYMENTS_INCOME
				));
				foreach ($points as $k1=>$v1){
					$userpoints += $v1['points'];
				}
				foreach ($grade as $k=>$v){
					if($userpoints >= $v['points_rule']){
						$val->membershipgrade_id = $v['id'];
					}
				}
				$val->save();
			}
			...
			foreach ($users as $key=>$val){
				...
				$criteria -> params[':user_id'] = $val['id'];
				$criteria -> addCondition('DATE_FORMAT(create_time,\'%Y\') = DATE_FORMAT(NOW(),\'%Y\')');
				...
				$points = UserPointsdetail::model()->findAll($criteria);
				foreach ($points as $k1=>$v1){
					$userpoints += $v1['points'];
				}
				foreach ($grade as $k=>$v){
	 ...
	 * $id  会员等级id
	 */
	public function editUserGradePoints($id,$points_ratio)
	{
		$result = array();
		$model = UserGradeDraft::model()->find('id=:id and flag=:flag',array(':id'=>$id,':flag'=>FLAG_NO));
		if($model->points_ratio != $points_ratio){
			$model->points_ratio = $points_ratio;
		}
	 ...
	 * $merchant_id 商家id
	 */
	public function setStoredPoints($merchant_id, $stored_points,$if_storedpay_get_points)
	{
		$result = array();
		$model = PointsRule::model()->find('merchant_id=:merchant_id and type=1',array(':merchant_id'=>$merchant_id));
		if (!empty($model)){
			if($model->stored_points != $stored_points){
				$model->stored_points = $stored_points;
			}
			if($model->if_storedpay_get_points != $if_storedpay_get_points){
				$model->if_storedpay_get_points = $if_storedpay_get_points;
			}
			$model -> last_time = date('Y-m-d H:i:s');
			if($model->save())
			{
	 ...
	 * $merchant_id 商家id
	 */
	public function getStoredPoints($merchant_id)
	{
		$model = PointsRule::model()->find('merchant_id=:merchant_id and type=1',array(':merchant_id'=>$merchant_id));
		return $model;
	}
	 ...
	 * $type  积分设置类型
	 */
	public function stopPointsSet($merchant_id,$type)
	{
		$result = array();
		$model = PointsRule::model()->find('merchant_id=:merchant_id and type=:type',array(':merchant_id'=>$merchant_id,':type'=>$type));
		$model->flag = FLAG_YES;
		$model -> last_time = date('Y-m-d H:i:s');
		if($model->save())
		{
	 ...
	 * $type  积分设置类型
	 */
	public function usePointsSet($merchant_id,$type)
	{
		$result = array();
		$model = PointsRule::model()->find('merchant_id=:merchant_id and type=:type',array(':merchant_id'=>$merchant_id,':type'=>$type));
		$model->flag = FLAG_NO;
	 ...
	 * $merchant_id 商家id
	 */
	public function setClearPoints($merchant_id, $cycle,$clean_start_time,$clean_date_type,$clean_date)
	{
		$result = array();
		$model = PointsRule::model()->find('merchant_id=:merchant_id and type=2',array(':merchant_id'=>$merchant_id));
		if (!empty($model)){
			if($model->cycle != $cycle){
				$model->cycle = $cycle;
			}
			if($model->clean_start_time != $clean_start_time){
				$model->clean_start_time = $clean_start_time;
			}
			if($model->clean_date_type != $clean_date_type){
				$model->clean_date_type = $clean_date_type;
			}
			if($model->clean_date != $clean_date){
				$model->clean_date = $clean_date;
			}			
			$model -> last_time = date('Y-m-d H:i:s');
			if($model->save())
			{
	 ...
	 * $merchant_id 商家id
	 */
	public function getClearPoints($merchant_id)
	{
		$model = PointsRule::model()->find('merchant_id=:merchant_id and type=2',array(':merchant_id'=>$merchant_id));
		return $model;		
	}
	 ...
	 * $merchant_id 商家id
	 */
	public function getAllClearPoints()
	{
		$models = PointsRule::model()->findAll('flag=:flag and type=2',array(':flag'=>FLAG_NO));
		return $models;
	}
	 ...
	 * 获取商户下会员的id数组
	 */
	public function getUserId($merchant_id)
	{
		$id_arr = array();
		$model = User::model()->findAll('merchant_id=:merchant_id and flag=:flag',array(':merchant_id'=>$merchant_id,':flag'=>FLAG_NO));
	 ...
	 * @return 
	 */
	public function clearUserPoints($user_id,$cleartime,$execute_time)
	{
		$result = array();
		$s1 = 0; //（第一次清理）清理时间前的积分收入
		$z1 = 0; //（第一次清理）执行时间前的积分总支出
		$s2 = 0; //（第一次清理）执行时间前的积分总收入
		$s3 = 0; //上一次结算时间到这次清理时间的收入
		$z2 = 0; //上一次结算时间到这次执行时间的总支出
		$s4 = 0; //上一次结算时间到这次执行时间的总收入
		$model = UserPointsdetail::model()->findAll('user_id=:user_id and `from`=:from',array(
				':user_id'=>$user_id,
				':from'=>USER_POINTS_DETAIL_FROM_CLEAN
		));
		if(!empty($model))
		{
			$criteria = new CDbCriteria();
			...
			$criteria -> params[':from'] = USER_POINTS_DETAIL_FROM_CLEAN;
			$criteria->order = 'create_time DESC';   // 降序排序
			$new = UserPointsdetail::model()->findAll($criteria);
			$last = $new[0];
			$jspoints = $last['user_points'];
			$lasttime = substr($last['create_time'], 0,10);
			

			//如果清理时间晚于上一次清理的时间就清理
			if ($cleartime>=$lasttime){
				//找出上一次结算时间到这次清理时间的收入
				$criteria = new CDbCriteria();
				$criteria -> addCondition('user_id = :user_id');
				$criteria -> params[':user_id'] = $user_id;
				$criteria -> addBetweenCondition('create_time',$lasttime,$cleartime);
				$criteria -> addCondition('balance_of_payments = :balance_of_payments');
				$criteria -> params[':balance_of_payments'] = BALANCE_OF_PAYMENTS_INCOME;
				$criteria -> addCondition('`from` != :from');
				$criteria -> params[':from'] = USER_POINTS_DETAIL_FROM_CLEAN;
				
				$model_s3 = UserPointsdetail::model()->findAll($criteria);
				
				foreach ($model_s3 as $k => $v){
					$s3+= $v['points'];
				}

				//找出上一次结算时间到这次执行时间的总收入
				$criteria = new CDbCriteria();
				$criteria -> addCondition('user_id = :user_id');
				$criteria -> params[':user_id'] = $user_id;
				$criteria -> addBetweenCondition('create_time',$lasttime,$execute_time);
				$criteria -> addCondition('balance_of_payments = :balance_of_payments');
				$criteria -> params[':balance_of_payments'] = BALANCE_OF_PAYMENTS_INCOME;
				$criteria -> addCondition('`from` != :from');
				$criteria -> params[':from'] = USER_POINTS_DETAIL_FROM_CLEAN;
				
				$model_s4 = UserPointsdetail::model()->findAll($criteria);
				
				foreach ($model_s4 as $k => $v){
					$s4+= $v['points'];
				}

				//找出上一次结算时间到这次执行时间的总支出
				$criteria = new CDbCriteria();
				$criteria -> addCondition('user_id = :user_id');
				$criteria -> params[':user_id'] = $user_id;
				$criteria -> addBetweenCondition('create_time',$lasttime,$execute_time);
				$criteria -> addCondition('balance_of_payments = :balance_of_payments');
				$criteria -> params[':balance_of_payments'] = BALANCE_OF_PAYMENTS_EXPEND;
				$criteria -> addCondition('`from` != :from');
				$criteria -> params[':from'] = USER_POINTS_DETAIL_FROM_CLEAN;
				
				$model_z2 = UserPointsdetail::model()->findAll($criteria);
				
				foreach ($model_z2 as $k => $v){
					$z2+= $v['points'];
				}
				var_dump($s3);
				var_dump($s4);
				var_dump($z2);
				//计算需要清理的时间端内，剩余积分
				$left1 = $jspoints + $s3 - $z2;
				var_dump($left1);
				if($left1 > 0){
					//在用户表减去清理时间段内未使用的剩余积分
					$model = User::model()->findByPk($user_id);
					$points = $model['points'] - $left1;
					$model->points = $points;
					$model -> last_time = date('Y-m-d H:i:s');
					$model->save();
					//在积分明细表生成因清理减去的积分记录
					$model1 = new UserPointsdetail();
					$model1->user_id = $user_id;
					$model1->points = $left1;
					$model1->user_points = $points;
					$model1->balance_of_payments = BALANCE_OF_PAYMENTS_EXPEND;
					$model1->from = USER_POINTS_DETAIL_FROM_CLEAN;
					$model1->create_time = date('Y-m-d H:i:s');
		...
		else
		{
			...
			$model_s1 = UserPointsdetail::model()->findAll('user_id=:user_id and create_time<=:cleartime and balance_of_payments=:balance_of_payments and `from`!=:from',array(
					':user_id'=>$user_id,
					':cleartime'=>$cleartime,
					':balance_of_payments'=>BALANCE_OF_PAYMENTS_INCOME,
					':from'=>USER_POINTS_DETAIL_FROM_CLEAN
			));
			foreach ($model_s1 as $k => $v){
				$s1+= $v['points'];
			}
			
			$model_s2 = UserPointsdetail::model()->findAll('user_id=:user_id and create_time<=:execute_time and balance_of_payments=:balance_of_payments and `from`!=:from',array(
					':user_id'=>$user_id,
					':execute_time'=>$execute_time,
					':balance_of_payments'=>BALANCE_OF_PAYMENTS_INCOME,
					':from'=>USER_POINTS_DETAIL_FROM_CLEAN
			));
			foreach ($model_s2 as $k => $v){
				$s2+= $v['points'];
			}
			$model_z1 = UserPointsdetail::model()->findAll('user_id=:user_id and create_time<=:execute_time and balance_of_payments=:balance_of_payments and `from`!=:from',array(
					':user_id'=>$user_id,
					':execute_time'=>$execute_time,
					':balance_of_payments'=>BALANCE_OF_PAYMENTS_EXPEND,
					':from'=>USER_POINTS_DETAIL_FROM_CLEAN
			));
			foreach ($model_z1 as $k => $v){
				$z1+= $v['points'];
			}
			$left = $s1 - $z1;
			var_dump($left);
	 ...
	 * @param unknown $authset
	 * @return string
	 */
	public function saveAuthSet($merchant_id,$authset){
		$result = array();
		$model = Merchant::model()->findByPk($merchant_id);
		if(!empty($model)){
			$model -> auth_set = $authset;
			$model -> last_time = date('Y-m-d H:i:s');
			if($model -> save()){
				$result['status'] = ERROR_NONE;
				$result['errMsg'] = '';
	 ...
	 * @return string
	 */
	public function getAuthSet($merchant_id){
		$result = array();
		$model = Merchant::model()->findByPk($merchant_id);
		$result = explode(',', $model['auth_set']);
		return $result;
	}
...
//        				$model['group_id'] = $tmp;
					...
					$userNum = Group::model()->count('group_id = :group_id and flag = :flag', array(
					    ':group_id' => $new_id,
                        ':flag' => FLAG_NO
                    ));
					UserGroup::model()->updateByPk($new_id, array('num' => $userNum));
    ...
    public function saveWechatFansInfo($merchant_id, $fromUsername)
    {
        ...
        try {
            ...
            $user = User::model()->find('wechat_id =:wechat_id and merchant_id =:merchant_id and type = :type', array(
            ...
            if (!empty($user)) {
                ...
                $new_user->from = ',' . USER_FROM_WECHAT . ',';
    ...
    {
        try {
            $user = User::model()->find('merchant_id =:merchant_id and wechat_id =:wechat_id and type = :type', array(
                ':merchant_id' => $merchant_id,
                ':wechat_id' => $fromUsername,
                ':type' => USER_TYPE_WECHAT_FANS
            ));
   ...
   public function getManageList($merchant_id)
   {
       ...
       //用户详情
        ...
        public function saveAlipayFansInfo($merchant_id,$fromUsername,$info){
            $result = array();
            try {
                //根据商户id和alipay openid 查找用户
                $user = User::model()->find('alipay_fuwu_id =:alipay_fuwu_id and merchant_id =:merchant_id and type = :type', array(
        ...
        * */
        public function cancelAlipaySubscribe($merchant_id,$fromUsername){
            try {
                $user = User::model()->find('merchant_id =:merchant_id and alipay_fuwu_id =:alipay_fuwu_id and type = :type', array(
                    ':merchant_id' => $merchant_id,
                    ':alipay_fuwu_id' => $fromUsername,
                    ':type' => USER_TYPE_ALIPAY_FANS
                ));
