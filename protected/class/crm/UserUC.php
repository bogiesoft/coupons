 ...
 * 会员类
 */
class UserUC extends mainClass
{
    /**
     * 获取商户id
     ...
     */
    public function userRegister($merchant_id, $account, $msg_pwd, $pwd, $confirm_pwd)
    {
        $result = array();  //返回值
        try {
            //参数验证
            ...
            $model->pwd = md5($pwd);
            $model->type = USER_TYPE_WANQUAN_MEMBER;
    ...
    public function checkAccount($encrypt_id, $account, $pwd)
    {
        ...
        try {
            ...
            if (empty($model)) {
                ...
                $result['data']['wechat_open_id'] = $model['wechat_id'];
                $result['data']['alipay_open_id'] = $model['alipay_fuwu_id'];
            }
...
// 					$data[$value['id']]['store_id'] =  $value['id'];
     ...
     * @param $coupons_id    优惠券id
     */
    public function newReceiveCoupons($user_id, $coupons_id, $marketing_activity_type = '', $marketing_activity_id = '')
    {
        $result = array();
    ...
    public function changeCode($card_id, $code, $friendUserName, $IsGiveByFriend, $OldUserCardCodee, $OuterId, $fromUsername)
    {
        ...
        try {
            if ($IsGiveByFriend == 1) { //转赠卡券
                ...
                $model = UserCoupons::model()->find('wechat_coupons_id = :wechat_coupons_id order by last_time DESC', array(':wechat_coupons_id' => $card_id));
                if (!empty($model)) {
                    $check_model = UserCoupons::model()->find('code =:code', array(':code' => $code));
                    if (!empty($check_model)) {
                        //创建优惠券核销码12位
                        $code_self = $this->getRandChar(12);
                        $usercode = UserCoupons::model()->find('code =:code', array(
                            ':code' => $code_self
                        ));
                        while (!empty($usercode)) {
                    ...
                    $model['code'] = $code;
                    $model['if_give'] = COUPONS_GIVE_NO;
                    $model['if_wechat'] = COUPONS_IF_WECHAT_YES;
// 	    				if ($IsGiveByFriend == 1){
// 	    					$model['if_give'] = 2;
// 	    					$model['give_openid'] = $friendUserName;
// 	    					$model['before_code'] = $OldUserCardCodee;
// 	    				}
                    $model['scene'] = $OuterId;
                    if ($model->save()) {
                        $transcation->commit(); //数据提交
...
//     	return $result;
    ...
    /** 根据open_id，merchant_id获取会员
     * @param $merchant_id
     * @param $wechat_open_id
     * @param $alipay_open_id
     * @return string
     */
    public function getUserByOpenid($merchant_id, $wechat_open_id, $alipay_open_id)
    {
        $result = array();
        try {
            if (!empty($wechat_open_id)) {
                $user = User::model()->find('merchant_id =:merchant_id and flag =:flag and wechat_id =:wechat_id and type =:type', array(
                    ':merchant_id' => $merchant_id,
                    ':flag' => FLAG_NO,
                    ':wechat_id' => $wechat_open_id,
                    ':type' => USER_TYPE_WANQUAN_MEMBER
                ...
                $user = User::model()->find('merchant_id =:merchant_id and flag =:flag and alipay_fuwu_id =:alipay_fuwu_id and type =:type', array(
                    ':merchant_id' => $merchant_id,
                    ':flag' => FLAG_NO,
                    ':alipay_fuwu_id' => $alipay_open_id,
                    ':type' => USER_TYPE_WANQUAN_MEMBER
     ...
     * @return string
     */
    public function fansUserRegister($user_id, $merchant_id, $account, $msg_pwd, $pwd, $confirm_pwd)
    {
        try {
            //参数验证
            if (empty($account) || !isset($account)) {
            ...
            //保存会员账号密码
            $model = User::model()->find('id =:id and flag =:flag', array(
                ':id' => $user_id,
                ':flag' => FLAG_NO
            ));
            ...
            $user_grade = UserGrade::model()->find('merchant_id = :merchant_id and if_default = :if_default', array(
                ':merchant_id' => $merchant_id,
