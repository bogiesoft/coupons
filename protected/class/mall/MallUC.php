...
class MallUC extends mainClass{
    /**
     * 查询商品列表
     * merchant_id 商户ID
     * placeholder 搜索框内容
       ...
       try {
           ...
           if(!empty($model)){
               ...
               foreach($model as $k=>$v)
               {
                   ...
                   $data[$k]['type']=$v['type'];
                   $data[$k]['img']=$v['img'];
                   $modelSku=ShopProductSku::model()->findAll('product_id=:product_id and flag=:flag',array(
            		':product_id'=>$v['id'],
            		':flag'=>FLAG_NO
                    ));
                    $minprice=array();
...
//            $criteria -> order = 'limit_num desc';
    	...
    	try {
    		...
    		if($cart){
    			foreach ($cart as $k => $v){
    				...
    				$data['list'][$k]['sku_name'] = $v -> sku -> name;
    				$data['list'][$k]['num'] = $v -> num;
    				$data['list'][$k]['sku_price'] = $v -> sku -> price;
    				$data['list'][$k]['sku_id'] = $v -> sku -> id;
    				$data['list'][$k]['img'] = $v -> product -> img;
    			}
    		}else{
    			$data['list'] = array();
    		}
    ...
    public function addCart($product_id,$sku_name,$num,$user_id){
    	...
    	try {
    		...
    		$sku = ShopProductSku::model() -> find('product_id=:product_id and name =:name and flag =:flag',array(
     ...
     * */
    public function getProductSkuInfo($arr_sku_id){
    	$result = array();
    	try {
    		$data = array();
    		$arr_sku = array();
    ...
    public function getGroup($merchant_id,$encrypt_id = '')
    {
        ...
        foreach($group as $k => $v)
        {
            ...
            $product = ShopProduct::model() -> findAll('group_id = :group_id and flag =:flag and status = :status',array(
            		':group_id' => $v['id'],
            		':flag' => FLAG_NO,
            		':status' => 2
            )); 
    ...
    {
    	$list = array();
    	$shop_product = ShopProduct::model()->findAll('merchant_id = :merchant_id  and flag = :flag and status =:status',array(
    			':merchant_id' => $merchant_id,
    			':flag' => FLAG_NO,
    ...
    public function getProductStandard($product_id){
    	...
    	try {
			$product = ShopProduct::model() -> findByPk($product_id);
                        $productSku = ShopProductSku::model()->findall('product_id=:product_id and flag=:flag',array(
                            ':product_id' => $product_id,
                            ':flag' => FLAG_NO,
                        ));                        
                        $minorgprice = array();
    ...
    public function getProductSkuPrice($product_id,$name){
    	...
    	try {
            $productSku = ShopProductSku::model()->find('product_id=:product_id and flag=:flag and name=:name',array(
                    ':product_id' => $product_id,
                    ':name' => $name,
                    ':flag' => FLAG_NO,
            ));
