   ...
   public function MallRefund($order_id, $order_sku_id, $order_status)
   {
	...
	public function wxpayRefund($refund_order_no) {
		...
		try {
			...
			$cmd->select('r.refund_money,o.order_paymoney, o.order_no, o.store_id, o.online_paymoney, k.num, k.price'); //查询字段
			...
			$total_money = $model['order_paymoney'];
			
			
			Yii::log('退款金额：'.$total_money.'&'.$refund_money,'warning');
			//微信接口请求
			...
			if ($result_code == _SUCCESS) {
				...
				Yii::log('天时退款：'.$order_no,'warning');
                //商城退款 第三方接口退款 
                $resu = $this->wappayNotifyUrl($order_no);
                Yii::log('天时退款2：'.$resu,'warning');
