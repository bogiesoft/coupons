...
// 			$criteria -> params[':order_status'] = $order_status;
        ...
        if (!empty($model)) {
            foreach ($model as $k => $v) {
                ...
                $refundRecord = RefundRecord::model() -> findAll('flag = :flag and order_id = :order_id',array(
                    ':flag' => FLAG_NO,
                    ':order_id' => $v -> id
                ));
                if(!empty($refundRecord)){
                    $data['list'][$k]['if_refund'] = 1;
                }else{
                    $data['list'][$k]['if_refund'] = 2;
                }
    ...
    public function getOrderSku($order_id)
    {
        ...
        if (!empty($model)) {
            foreach ($model as $k => $v) {
                ...
                $array[$k]['consume_num'] = $v->consume_num; //价格
...
// 			));
    ...
    * */
     ...
     * $consume_num  新增核销数量
     */
    public function editOrderSkuConsumeNum($order_no,$consume_num){
        $result = array();
        try {
            $order = Order::model() -> find('order_no = :order_no and flag = :flag',array(
                ':order_no' => $order_no,
            ...
            $orderSku = OrderSku::model() -> find('order_id =:order_id and flag = :flag',array(
                ':order_id' => $order -> id,
                ':flag' => FLAG_NO
            ));
            
            $orderSku -> consume_num += $consume_num;
            if($orderSku -> update()){
                $result['status'] = ERROR_NONE;
            }else{
    ...
    public function editOrderThridPartyCode($order_no,$code){
        $result = array();
        try {
            $order = Order::model() -> find('order_no = :order_no and flag = :flag',array(
                ':order_no' => $order_no,
