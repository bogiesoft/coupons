...
class DShopC extends mainClass
{
    //商品分页
    public $page = null;
    //商品编辑分页
        ...
        try {
            ...
            if (!empty($model)) {
                ...
                foreach ($model as $key => $value) {
                    ...
                    //商品数
                    $model_count = DProduct::model()->findAll('merchant_id=:merchant_id and
...
// 				$criteria->params = array(':group_id'=>$group_id);
            ...
            //显示分页
            $pages = new CPagination(DProduct::model()->count($criteria));
            $pages->pageSize = Yii::app()->params['perPage'];
            $pages->applyLimit($criteria);
            $this->page = $pages;

            $product = DProduct::model()->findAll($criteria);
            $data = array();
            if ($product) {
                foreach ($product as $k => $v) {
                    ...
                    $count_buyed_num = 0;
                    $count_kucun = 0;
                    $product_sku = DProductSku::model()->findAll('product_id=:product_id and flag = :flag', array(
                        ':product_id' => $v->id,
                        ':flag' => FLAG_NO
                    ));
    ...
    public function saveProStandard($name, $content, $save_type, $model_id, $merchant_id)
    {
        ...
        try {
            ...
            $standardtemplet['create_time'] = date("Y-m-d H:i:s");
            $standardtemplet['last_time'] = date("Y-m-d H:i:s");
    ...
    public function saveProduct($id, $merchant_id, $category_id, $group_id, $standard, $page_id, $type, $name, $price, $img_arr, $freight_type, $freight_money, $freight_id, $if_invoice, $sku_arr, $limit_num, $brief_introduction, $detailed_introduction, $if_show_num, $stock_num, $third_party_source, $third_party_product_id, $use_time_type, $date_num, $check_time_type, $check_day, $check_hour, $check_minute)
    {
        ...
        try {
            if (isset($id) && $id) {
                ...
                DProductSku::model()->updateAll(array("flag" => FLAG_YES), "product_id = :product_id", array(':product_id' => $id));
            } else {
                $model = new DProduct();
            }
            ...
            $model['use_time_type'] = $use_time_type;
            $model['date_num'] = $date_num;
            ...
            $model['third_party_source'] = $third_party_source;
            $model['third_party_product_id'] = $third_party_product_id;
            //保存图片
            if (!empty($img_arr)) {
                $img = "";
            ...
            $model['check_time_type'] = $check_time_type;
            $model['check_day'] = $check_day;
            $model['check_hour'] = $check_hour;
            $model['check_minute'] = $check_minute;
    ...
    public function getProduct($id)
    {
        ...
        $list = array();
        try {
            $pro_model = DProduct::model()->findByPk($id);
            $sku_model = DProductSku::model()->findAll(array(
...
//     		exit();
            ...
            if (!empty($sku_model)) {
                foreach ($sku_model as $key => $value) {
                    ...
                    $list['pro_sku'][$value['id']]['merchant_no'] = $value['merchant_no'];
                    $num += $value['num'];
                }
            }
            $list['num'] = $num;
            ...
            $list['third_party_product_id'] = $pro_model['third_party_product_id'];
            $list['third_party_source'] = $pro_model['third_party_source'];
            $list['use_time_type'] = $pro_model['use_time_type'];
            $list['date_num'] = $pro_model['date_num'];
            $list['check_time_type'] = $pro_model['check_time_type'];
            $list['check_day'] = $pro_model['check_day'];
            $list['check_hour'] = $pro_model['check_hour'];
            $list['check_minute'] = $pro_model['check_minute'];
...
//                $criteria->compare('name',$name,true);
        ...
        try {
            ...
            if (!empty($del_arr)) {
                ...
                for ($i = 0; $i < count($del_arr); $i++) {
                    $model = DProduct::model()->findByPk($del_arr[$i]);
                    if ($model) {
                        $model->flag = FLAG_YES;
                        if ($model->update()) {
        ...
        try {
            ...
            if (!empty($uc_arr)) {
                ...
                for ($i = 0; $i < count($uc_arr); $i++) {
                    $model = DProduct::model()->find('id=:id and merchant_id=:merchant_id', array(':merchant_id' => $merchantId, ':id' => $uc_arr[$i]));
    ...
    public function ChangeGroup($merchant_id, $group_id, $shop_id)
    {
        ...
        try {
            ...
            $model = DProduct::model()->findByPk($shop_id);
            $model->group_id = $group_id;
            if ($model->update()) {
                $result['status'] = ERROR_NONE;
                $result['errMsg'] = '';
    ...
    //添加营销活动
    public function addDmallActivity($merchant_id, $type, $name, $num, $start_time, $end_time, $coupon_id, $img, $original_price = NULL)
    {
        $result = array();
        try {
            $mallActivity = new MallActivity();
            $mallActivity->merchant_id = $merchant_id;
            $mallActivity->type = $type;
            $mallActivity->name = $name;
            $mallActivity->num = $num;
            $mallActivity->img = $img;
            $mallActivity->start_time = $start_time;
            $mallActivity->end_time = $end_time;
            $mallActivity->coupons_id = $coupon_id;
            $mallActivity->original_price = $original_price;
            $mallActivity->status = DMALL_ACTIVITY_STATUS_NOT_START;
            $mallActivity->create_time = date('Y-m-d H:i:s');
            if ($mallActivity->save()) {

            } else {
                throw new Exception('营销活动添加失败');
            }
            $result['status'] = ERROR_NONE;
    ...
    //获取营销活动列表
    public function getDmallActivityList($merchant_id)
    {
        $result = array();
        try {
            ...
            $criteria->order = 'create_time desc';
            //显示分页
            $pages = new CPagination(MallActivity::model()->count($criteria));
            $pages->pageSize = Yii::app()->params['perPage'];
            $pages->applyLimit($criteria);
            $this->page = $pages;

            $mallActivity = MallActivity::model()->findAll($criteria);

            $data = array();
            foreach ($mallActivity as $k => $v) {
                $coupon = Coupons::model() -> findByPk($v -> coupons_id);
                ...
                if ($v->status == DMALL_ACTIVITY_STATUS_NOT_START) {
                    ...
                    if (strtotime($v->end_time) < strtotime(date('Y/m/d H:i'))) {
                        $v->status = DMALL_ACTIVITY_STATUS_END;
                    } elseif ($coupon->num - $coupon->get_num <=0 ) {
                ...
                $data[$k]['receive_num'] = $coupon->get_num;
                $data[$k]['coupon_num'] = $coupon->num;
            }
    ...
    //删除营销活动
    public function delDMallActivity($id)
    {
        $result = array();
        try {
            $activity = MallActivity::model()->findByPk($id);
            if ($activity) {
                $activity->flag = FLAG_YES;
                if ($activity->update()) {
                    $result['status'] = ERROR_NONE;
                }
            } else {
    ...
    //选择优惠券 并返回该券的库存
    public function getCouponStock($coupon_id){
        $result = array();
        try {
            $coupon = Coupons::model() -> findByPk($coupon_id);
            if ($coupon) {
                $result['status'] = ERROR_NONE;
                $result['data'] = $coupon -> num - $coupon -> get_num;
            } else {
