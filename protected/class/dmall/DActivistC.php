...
class DActivistC extends mainClass{
	 ...
	 * 获取维权订单列表
	 */
	public function getActivistList($merchant_id)
	{
		$result = array ();
		...
		if(!empty($model)){
			foreach ($model as $k=>$v){
                    ...
                    if(!empty($order_sku)) {
                        ...
                        $data['list'][$k]['user_name'] = isset($arr[0]) ? $arr[0] : ''; //买家名称
                        $data['list'][$k]['user_phone'] = isset($arr[1]) ? $arr[1] : ''; //买家手机号
...
//                $data['refund_address'] = $model -> refund_address; //退款地址
	    	...
	    	$data['order_info'] = $this->getOrderInfo($order_sku -> order_id); //获取sku对应的订单详情
                $dproduct = DProduct::model()->find('id=:id and flag=:flag',array(
                    ':flag' => FLAG_NO,
                    ':id'   => $order_sku -> dshop_product_sku -> product_id,
                ));
	    	$data['shopProductSku_name'] = $dproduct['standard'];
	    	$sku_name=explode(';',$data['shopProductSku_name']);
                $color = explode(':',$sku_name[0]);
                if($color[1]){
