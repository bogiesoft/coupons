...
// 			}
	...
	public function orderPaySuccess($order_no, $pay_time, $trade_no, $alipay_account, $merchant_discount=NULL, $alipay_discount=NULL, $alipay_user_id=NULL, $wxpay_openid=NULL, $wxpay_p_openid=NULL, $ums_card_no=NULL) {
		...
		if (!empty($model['user_id']) && $model['if_has_points'] == IF_HAS_POINTS_YES) {
			if ($model['pay_channel'] == ORDER_PAY_CHANNEL_STORED) {
				$merchant_id = $model['merchant_id'];
				//查询储值支付是否获得积分
				$rule = PointsRule::model()->find('merchant_id = :merchant_id and flag = :flag and type = :type',
						array(':merchant_id' => $merchant_id, ':flag' => FLAG_NO, ':type' => '1'));
				if (!empty($rule)) {
		...
		//订单状态查询：防止同步和异步的订单修改操作重复
		$tmp_model = Order::model()->findByPk($model['id']);
		if ($tmp_model['pay_status'] == ORDER_STATUS_PAID) {
			$data = array('status' => ERROR_DUPLICATE_DATA);
			return $data;
		}
...
//     			}
     ...
     * @throws Exception
     */
    public function wechatNotice($order_no) {
    	//订单查找
    	$criteria = new CDbCriteria();
    	$criteria->addCondition('order_no = :order_no');
    	$criteria->params[':order_no'] = $order_no;
