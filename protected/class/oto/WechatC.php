 ...
 * 微信公众号类
 */
class WechatC extends mainClass
{
    /**
     * 获取系统地址
            ...
            if (!empty($model)) {
                ...
                if (!empty($coupons)) {
                    foreach ($coupons as $key => $value) {
                        //$redirect_uri = USER_DOMAIN_COUPONS . '/getCoupon?encrypt_id=' . $model['encrypt_id'] . '&coupon_id=' . $value['id'] . '&source=wechat';
...
//      $filepath = 'D:/www/workplace/kuaiguanjia/upload/images/gj/source/'.$img;
    ...
    public function uploadNews($material, $img_list, $access_token)
    {
        ...
        foreach ($material as $key => $value) {
            if($value['jump_type'] == MATERIAL_CONTENT_TEXT) {
                $content_source_url = $value['link_content'];
            }
            if($value['jump_type'] == MATERIAL_CONTENT_URL) {
                $content_source_url = $value['link_content'];
            }
            if ($value['jump_type'] == MATERIAL_CONTENT_CUSTOMIZE_URL) {
                $content_source_url = $value['customize_link'];
            }
            $news[] = array(
    ...
    public function saveUserinfo($id, $userinfo_re, $data)
    {
        ...
        //refresh_token 30天缓存
        $refresh_token = Yii::app()->memcache->get($userinfo_re['openid'] . '_user_refresh_token');
        Yii::app()->memcache->set($userinfo_re['openid'] . '_userinfo_refresh_token', $refresh_token, 2592000);
        //修改最后更新时间
