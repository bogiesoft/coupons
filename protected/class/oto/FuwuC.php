...
class FuwuC extends mainClass
{
    ...
    public function getSystemUrl($merchant_id)
    {
        ...
        try {
            ...
            if (!empty($model)) {
                $list[0] = "--请选择--";
                $member_center = USER_DOMAIN . '/memberCenter?encrypt_id=' . $model['encrypt_id'];
                $shop = WEB_APP_DOMAIN . '/myun/Yunshop/shop?encrypt_id=' . $model['encrypt_id'] . '&source=wechat';
                $book_operate = WEB_APP_DOMAIN . '/book/Book/bookOperate?encrypt_id=' . $model['encrypt_id'] . '&source=wechat';
                $order = USER_DOMAIN . '/orderList?encrypt_id=' . $model['encrypt_id'] . '&stored_confirm_status=' . ORDER_PAY_WAITFORCONFIRM;
                ...
                if (!empty($coupons)) {
                    foreach ($coupons as $key => $value) {
                        ...
                        $url = USER_DOMAIN_COUPONS . '/getCoupon?qcode=' . $value['code'];
...
// 					':from_platform' =>FROM_PLATFORM_ALI,
     ...
     * @param  $rate
     */
    public function editMaterial($id, $title, $link_content, $cover_img, $jump_type, $content, $customize_link)
    {
        $result = array();
        try {
            ...
            $model['customize_link'] = $customize_link;
            //参数验证
     ...
     * @param  $material_id          素材id
     * @param  $rate                 排序值
     * @param $customize_link        自定义链接
     * @return array
     */
    public function saveSingleMaterial($model, $merchant_id, $title, $cover_img, $abstract, $jump_type, $content, $link_content, $material_id, $rate, $customize_link)
    {
        $result = array();
        try {
...
// 			$model['from_platform'] = FROM_PLATFORM_ALI;
            ...
            if ($jump_type == MATERIAL_CONTENT_TEXT) {
                $model['content'] = $content;
            } else if($jump_type == MATERIAL_CONTENT_URL){
                $model['link_content'] = $link_content;
            } else if($jump_type == MATERIAL_CONTENT_CUSTOMIZE_URL){
                $model['customize_link'] = $customize_link;
            }
            //参数验证
            if (!isset($title)) {
...
// 									"imageUrl" => 'D:/www/workplace/kuaiguanjia/upload/images/gj/source/'.$material['cover_img'],
    ...
    public function saveUserInfo($id, $userinfo_re, $data)
    {
        ...
        //alipay_user_id 30天缓存
        Yii::app()->memcache->set($userinfo_re->alipay_user_id . '_userinfo_alipay_user_id', $userinfo_re->alipay_user_id, 2592000);
